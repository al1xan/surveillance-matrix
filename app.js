 ...
 * Official Brand SVG Icons (Local Repo Assets)
 */
...
// Official Brand Color Map
const appBrandColors = {
    whatsapp: 'linear-gradient(135deg, #25D366, #128C7E)',
    instagram: 'linear-gradient(45deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888)',
    tiktok: '#000000',
    gmail: 'linear-gradient(135deg, #ea4335, #c5221f)',
    chrome: 'linear-gradient(135deg, #4285F4, #34A853, #FBBC05, #EA4335)',
    linkedin: '#0A66C2',
...
// ────────────────────────────────────────────────────────────────────────────────
// Official Brand Icon Renderer
// ────────────────────────────────────────────────────────────────────────────────

function getAppOfficialLogo(app) {
    const id = (app.id || '').toLowerCase();
    const bg = appBrandColors[id] || 'rgba(255, 255, 255, 0.08)';

    // Check if local SVG icon exists for standard preset apps
    if (appBrandColors[id]) {
        return `
            <div class="brand-icon-box" style="background: ${bg};">
                <img src="./assets/icons/${id}.svg" alt="${app.name}" style="width: 24px; height: 24px; filter: brightness(0) invert(1);" onerror="this.outerHTML='${app.icon || '📱'}';">
            </div>
        `;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    // Absolute failsafe: Immediately remove any language selector bar if cached HTML is served
...
function renderAppGrid() {
    ...
    filteredApps.forEach((app, idx) => {
        ...
        const iconHtml = getAppOfficialLogo(app);
...
function showTosdrModal(appId) {
    ...
    if (modalBody) {
        ...
        const iconHeader = getAppOfficialLogo(app);
